...
spec sheets on superlift.net: 114 models on 10 of its 15 lines. Its ground level
...
generator keeps one row per model number, so 117 typed rows become 114 on the
site, with those three appearing once. Its stainless economy sheet prints the SL B SS50 raised height as both
